<?php
    ...
    if (isset($_POST['attendanceReq'])) {
        ...
        $excuseLetter = $obj_Enrollment->sanitizeInput($_POST['excuseLetter']);  // include excuse letter
        

        // * Future update: dedicated cut-off time per course
        $remarks = "";
        ...
        // to be inserted in attendance table
        $attendanceData = [
            "year_level" => $year_level,
            "student_username" => $student_username,
            "course_title" => $course_title,
        ...
        // to be inserted in excuse letters table
        // does not execute if student is present
    ...
    if (isset($_POST['filterReq'])) {
        ...
        if (empty($course) && empty($year_level) && empty($attendance_Status)) {
            $result = $obj_Attendance->getAll();
        } else {
    ...
    if (isset($_POST['updateLetterStatusReq'])) {
        $letter_id = $obj_ExcuseLetter->sanitizeInput($_POST['letter_id']);
        $newStatus = $obj_ExcuseLetter->sanitizeInput($_POST['letter_status']);

        $result = $obj_ExcuseLetter->updateLetterStatus($letter_id, $newStatus);
