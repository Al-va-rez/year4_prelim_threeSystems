...
<body>
    ...
    <form id="createAttendanceForm" class="flex flex-col w-1/2 gap-4 items-center my-4">
        ...
        <!-- show only when #formAttendanceStatus is set to "Absent" -->
                ...
                <tr class="text-center">
                    ...
                    <td class="border p-2">
                        ...
                        <?php if (empty($a['remarks'])): ?>
                            ...
                            <!-- show status if already submitted excuse letter and status has been updated -->
                            <?php $letter_status = $obj_ExcuseLetter->getLetter($_SESSION['username'], $a['course_title'], $a['date_filed']); ?>
                            <?php if ($letter_status['status'] == "Approved"): ?>
                                Excuse letter status: <span class="text-green-600"><?= $letter_status['status'] ?></span>
                            <?php elseif ($letter_status['status'] == "Denied"): ?>
                                Excuse letter status: <span class="text-red-500"><?= $letter_status['status'] ?></span>
                            <?php else: ?>
                                Excuse letter status: <span class="text-yellow-600"><?= $letter_status['status'] ?></span>
                            <?php endif ?>

                            <!-- otherwise, just show button for viewing the submitted excuse letter -->
                            <form class="viewExcuseLetter mt-4">
                        ...
                        <!-- if present (output: "On time" or "Late") -->
