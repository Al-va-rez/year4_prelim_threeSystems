...
class User extends Database {
     ...
     */
    public function registerUser($username, $email, $password, $contact_number, $is_client = 0, $is_admin = 1) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO fiverr_clone_users (username, email, password, is_client, is_admin, contact_number) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            $this->executeNonQuery($sql, [$username, $email, $hashed_password, $is_client, $is_admin, $contact_number]);
            return true;
    ...
    public function loginUser($email, $password) {
        $sql = "SELECT user_id, username, password, is_admin FROM fiverr_clone_users WHERE email = ?";
