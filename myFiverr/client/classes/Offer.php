...
class Offer extends Database {
    ...
    public function clientAlreadyOffered($user_id, $proposal_id) {
        $sql = "SELECT COUNT(*) AS cnt FROM offers WHERE user_id = ? AND proposal_id = ?";
        $row = $this->executeQuerySingle($sql, [$user_id, $proposal_id]);
        $count = (int) $row['cnt'];
        return $count > 0;
    }
}
