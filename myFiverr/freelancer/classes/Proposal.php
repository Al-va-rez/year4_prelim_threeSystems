...
class Proposal extends Database {
     ...
     */
    public function createProposal($user_id, $main_category_title, $subcategory_title, $description, $image, $min_price, $max_price) {
        $sql = "INSERT INTO Proposals (user_id, category_title, subcategory_title, description, image, min_price, max_price) VALUES (?, ?, ?, ?, ?, ?, ?)";
        return $this->executeNonQuery($sql, [$user_id, $main_category_title, $subcategory_title, $description, $image, $min_price, $max_price]);
    }
    ...
    public function getProposalsByCategory($category) {
        $sql = "SELECT
                    Proposals.*,
                    fiverr_clone_users.*, 
                    Proposals.date_added AS proposals_date_added
                ...
                ORDER BY Proposals.date_added DESC";
        return $this->executeQuery($sql, [$category]);
    }

    public function getProposalsBySubcategory($subcategory) {
        $sql = "SELECT
                    Proposals.*,
                    fiverr_clone_users.*, 
                    Proposals.date_added AS proposals_date_added
                FROM Proposals
                JOIN fiverr_clone_users ON Proposals.user_id = fiverr_clone_users.user_id
                WHERE subcategory_title = ?
                ORDER BY Proposals.date_added DESC";
        return $this->executeQuery($sql, [$subcategory]);
    }
